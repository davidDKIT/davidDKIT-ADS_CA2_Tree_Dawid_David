...
/// <returns></returns>
std::string getRandomLowercase(int minLength, int maxLength) {
	if (minLength == 0 || maxLength == 0 || maxLength < minLength)
		throw std::invalid_argument::invalid_argument("One or more parameters is incorrect!");
	...
	std::string strOut = to_string(pCharArr, randLength);
